...
export default function StudentView({
  ...
  // 待機画面（プロジェクターモード時はスキップ）
  if (hasSubmitted && currentSlideMode === 'answer' && !isProjector) {
    return (
  ...
  // 表示モード or 結果モード
  return (
    <div className={isProjector ? 'w-full h-full' : 'max-w-4xl mx-auto'}>
      <div className={cn(isProjector ? 'h-full flex flex-col' : 'bg-white rounded-lg shadow-lg ', 'p-6')}>
        {/* ヘッダー（プロジェクターモード時は最小限に） */}
        {!isProjector && (
          <div className="mb-6">
        ...
        {/* スライド内容 */}
        {currentSlide ? (
          <div className={isProjector ? ' flex items-center justify-center  ' : 'space-y-6'}>
            <div className={isProjector ? 'w-full max-w-7xl' : 'w-full'}>
              {/* ノーマルスライドのブロック表示 */}
              ...
              {currentSlide.templateType === 'psycho' && (
                <div>
                  <PsychoAnswerForm
                    ...
                    existingAnswer={
                      !isProjector && student
                        ? game.Slide.find((s: any) => s.id === currentSlide.id)?.SlideAnswer?.find(
                          (a: any) => a.studentId === student.id
                        )
                        : undefined
                    }
                    onSubmit={handlePsychoSubmit}
                    isReadOnly={hasSubmitted || currentSlideMode === 'view' || isProjector}
                ...
                <div className="border-t pt-6">
                  {isProjector ? (
                    <div className="text-center py-12 text-gray-500">
                      ...
                      <p className="text-xl font-bold">生徒の回答を受け付けています</p>
                      <p className="mt-2">お手元の端末で回答してください</p>
                    </div>
                  ) : (
                    <>
                ...
                <div className="border-t pt-6">
                  ...
                  {currentSlide.templateType === 'choice' && (
                    <div className="mb-4">
                      ...
                      {/* 自分の回答の正誤（プロジェクターモード時は非表示） */}
                      {!isProjector && answerData && currentSlide.contentData?.choices && (
                        <div className="mt-4">
                  ...
                  {/* やり直しボタン（回答済みの場合、プロジェクターモード時は非表示） */}
                  {hasSubmitted && currentSlide.templateType !== 'psycho' && !isProjector && (

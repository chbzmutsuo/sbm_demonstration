...
const callGeminiAPI = async (prompt: string): Promise<any> => {
  if (!GEMINI_API_KEY) {
    throw new Error('Gemini API Keyが設定されていません。環境変数NEXT_PUBLIC_GEMINI_API_KEYを設定してください。')
  }
...
export const analyzeWithAI = async (
  ...
  try {
    // Step 1: 観点提案
    const viewpoints = await proposeViewpoints(messages)

    // Step 2: 事実分析
    const factAnalysis = await analyzeFacts(messages, viewpoints.viewpoints)

    // Step 3: 行動提案
    const actionSuggestions = await suggestActions(factAnalysis)
